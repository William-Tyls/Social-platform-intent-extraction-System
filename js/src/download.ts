...
export async function fetchChecksums(version?: string): Promise<Map<string, string> | null> {
  ...
  // Checksums are ALWAYS fetched from GitHub releases, regardless of
  // CLOAKBROWSER_DOWNLOAD_URL (which only governs the binary tarball host).
  // Using a custom binary host must not silently disable checksum verification
  // — that would let a tampered mirror serve any file unchallenged.
  const urls = hasCustomUrl
    ? [`${GITHUB_DOWNLOAD_BASE_URL}/chromium-v${v}/SHA256SUMS`]
    : [
        `${DOWNLOAD_BASE_URL}/chromium-v${v}/SHA256SUMS`,
        `${GITHUB_DOWNLOAD_BASE_URL}/chromium-v${v}/SHA256SUMS`,
      ];
...
async function extractTar(archivePath: string, destDir: string): Promise<void> {
  await tarExtract({
    ...
    filter: (entryPath: string, entry: any) => {
      // Reject path traversal / absolute paths outright — silently skipping
      // lets a malicious archive smuggle content past the filter, so throw.
      if (path.isAbsolute(entryPath) || entryPath.includes("..")) {
        throw new Error(
          `[cloakbrowser] Refusing to extract archive entry with suspicious path: ${entryPath}`
        );
      }
      // Symlinks can escape destDir via an absolute linkpath or one containing
      // ".." — reject those too (audit 4.5).
      if (entry && entry.type === "SymbolicLink" && entry.linkpath) {
        const link = String(entry.linkpath);
        if (path.isAbsolute(link) || link.includes("..")) {
          throw new Error(
            `[cloakbrowser] Refusing to extract symlink with suspicious target: ${entryPath} -> ${link}`
          );
        }
      }
      return true;
...
/**
 * Escape a string for safe interpolation into a PowerShell single-quoted string.
 * PowerShell single-quotes treat everything literally except a literal single
 * quote, which is doubled to escape it. This prevents command injection via
 * archive/dest paths reaching the `-Command` argument.
 */
...
/**
 * Walk every extracted file under destDir and confirm its realpath stays inside
 * destDir. Symlinks or oddly-named entries that escaped the per-entry filters
 * would resolve outside the tree and are rejected here as a backstop.
 */
function verifyNoTraversal(destDir: string): void {
  const resolvedDest = fs.realpathSync(destDir);
  const entries = fs.readdirSync(destDir, { recursive: true }) as string[];
  for (const rel of entries) {
    ...
    try {
      ...
      // Entry vanished or is unreadable — nothing to verify.
      continue;
    }
    if (resolved !== resolvedDest && !resolved.startsWith(resolvedDest + path.sep)) {
      throw new Error(
        `[cloakbrowser] Refusing archive entry that resolves outside the destination directory: ${rel} -> ${resolved}`
      );
    }
  }
}
...
async function extractZip(archivePath: string, destDir: string): Promise<void> {
  ...
  if (process.platform === "win32") {
    ...
    // Paths are single-quote-escaped via psEscape to prevent command injection.
    execFileSync("powershell", [
      "-NoProfile", "-Command",
      `Add-Type -AssemblyName System.IO.Compression.FileSystem; ` +
      `[System.IO.Compression.ZipFile]::ExtractToDirectory('${psEscape(archivePath)}', '${psEscape(destDir)}')`,
    ], { timeout: 120_000 });
  } else {
    execFileSync("unzip", ["-o", archivePath, "-d", destDir], { timeout: 120_000 });
  }

  // Backstop: confirm nothing escaped destDir (zip filter is external here).
  verifyNoTraversal(destDir);
}
...
export async function checkWrapperUpdate(): Promise<void> {
  ...
  try {
    ...
    // Non-fatal — never block binary update check. Wrapper update probes the
    // npm registry which can flap; staying silent here is intentional (covered
    // by the "silent on network error" test).
  }
}
...
async function checkAndDownloadUpdate(): Promise<void> {
  try {
    ...
    // Background update failed — surface it (don't silently swallow), but
    // don't disrupt the user's launch either.
    console.warn("[cloakbrowser] Background update failed:", err);
  }
}

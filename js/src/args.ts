...
export function buildArgs(options: LaunchOptions): string[] {
  ...
  if (options.extensionPaths?.length) {
    // 校验:路径须是已存在目录,且不含逗号(逗号会破坏 --load-extension 解析)
    const absPaths = options.extensionPaths.map((p) => {
      const abs = path.resolve(p);
      if (abs.includes(",")) {
        throw new Error(`extension path cannot contain comma: ${abs}`);
      }
      if (!fs.existsSync(abs) || !fs.statSync(abs).isDirectory()) {
        throw new Error(`extension path is not a directory: ${abs}`);
      }
      return abs;
    });

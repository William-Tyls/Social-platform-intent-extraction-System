...
// Lazily evaluated on first access so importing the module no longer performs
// a synchronous filesystem read at import time (audit 3.5).
let _wrapperVersion: string | null = null;

/** Lazily read the wrapper version from package.json (cached after first call). */
export function getPkgVersion(): string {
  if (_wrapperVersion !== null) return _wrapperVersion;
  try {
    ...
    _wrapperVersion = "0.0.0";
  }
  return _wrapperVersion;
}

/**
 * Wrapper version. Reads package.json lazily on first access via a Proxy so
 * that merely importing this module does not trigger a synchronous file read.
 * Coerces to the underlying string for all read-only uses (comparisons,
 * interpolation, JSON). Use `getPkgVersion()` to get the string directly.
 */
export const WRAPPER_VERSION: string = new Proxy(
  // Empty target — every access is delegated to the lazily-resolved string.
  // We deliberately keep the target minimal; string coercion is handled by
  // the `Symbol.toPrimitive` / `toString` traps below.
  { [Symbol.toPrimitive]: () => getPkgVersion() },
  {
    get: (_t, prop) => {
      ...
      if (typeof prop === "symbol") {
        ...
        return Reflect.get(v as unknown as object, prop);
      }
      return Reflect.get(v as unknown as object, prop);
    },
    getPrototypeOf: () => String.prototype,
    ownKeys: () => Reflect.ownKeys(getPkgVersion() as unknown as object),
    getOwnPropertyDescriptor: (_t, prop) =>
      Reflect.getOwnPropertyDescriptor(getPkgVersion() as unknown as object, prop),
  },

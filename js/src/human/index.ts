...
function patchPage(page: Page, cfg: HumanConfig, cursor: CursorState): void {
  ...
  async function ensureCursorInit(): Promise<void> {
    ...
    if (!initPromise) {
      ...
      // Swallow rejection on the cached promise so a failure doesn't leave a
      // rejected promise that future callers would re-await and re-throw.
  ...
  // Routed through ensureCursorInit so the init is cached and concurrent
  // callers don't race it.

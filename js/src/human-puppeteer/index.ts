...
function patchPage(page: Page, cfg: HumanConfig, cursor: CursorState): void {
  ...
  // Cache the in-flight cursor init so concurrent callers (e.g. an immediate
  // page.click() racing the fire-and-forget init below) don't each run a
  // separate mouseMove and clobber cursor.x/y. Only the first caller runs.
  ...
  // Initialize cursor. Routed through ensureCursorInit so the init is cached
  // and concurrent callers don't race it.
  ensureCursorInit().catch(() => {});
...
export function patchBrowser(browser: Browser, cfg: HumanConfig): void {
  ...
  // Non-async handler: resolving target.page() is async, so do it via .then()
  // and swallow rejections. An async handler would surface rejected promises
  // as unhandledRejection events on the process.
  browser.on('targetcreated', (target: any) => {

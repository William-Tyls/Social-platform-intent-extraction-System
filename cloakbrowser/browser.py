...
def _prepare_launch(
    ...
    """所有 launch 函数共用的参数编排,返回 (binary_path, chrome_args, proxy_kwargs)。

    把原先在 6 个 launch 函数里逐字重复的
    ensure_binary → maybe_resolve_geoip → _resolve_proxy_config →
    _resolve_webrtc_args → 注入 --fingerprint-webrtc-ip → build_args
    编排集中到一处,避免改流程时漏改某个函数。
    """
    ...
    if exit_ip and not (args and any(a.startswith("--fingerprint-webrtc-ip") for a in args)):
        args = list(args or []) + [f"--fingerprint-webrtc-ip={exit_ip}"]
    chrome_args = build_args(
        stealth_args, (args or []) + proxy_extra_args,
        timezone=timezone, locale=locale, headless=headless, extension_paths=extension_paths,
    )
    return binary_path, chrome_args, proxy_kwargs
...
def launch(
    ...
    **kwargs: Any,
    ...
    binary_path, chrome_args, proxy_kwargs = _prepare_launch(
        stealth_args, args, proxy, timezone, locale, geoip, headless, extension_paths)
...
def launch_context(
    ...
    **kwargs: Any,
    ...
    # geoip/webrtc 编排统一由 launch() 内部的 _prepare_launch 处理,此处透传 geoip。
    # --fingerprint-timezone 是进程级(渲染器读 CommandLine),对所有 context 生效;
    # locale/timezone 只走二进制 flag,不用 CDP emulation(可被检测)。
    browser = launch(headless=headless, proxy=proxy, args=args, stealth_args=stealth_args,
                     timezone=timezone, locale=locale, backend=backend, geoip=geoip,
...
async def launch_context_async(
    ...
    **kwargs: Any,
    ...
    # geoip/webrtc 编排统一由 launch_async() 内部的 _prepare_launch 处理,此处透传 geoip。

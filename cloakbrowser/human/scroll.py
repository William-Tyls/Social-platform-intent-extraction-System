...
from .config import HumanConfig, rand, rand_range, rand_int_range, rand_lognormal, rand_unit, sleep_ms
...
except Exception:  # pragma: no cover — playwright not installed at lint time
    _TargetClosedError = ()  # type: ignore[assignment]
...
def _get_element_box(page: Any, selector: str, timeout: float = 30000) -> Optional[dict]:
    ...
    A ``TargetClosedError`` (page/navigation gone) is re-raised so callers can
    abort cleanly; timeouts and other transient errors return ``None`` with a
    debug log so the caller's retry loop can keep going.
    """
    try:
        ...
        logger.debug(
            "_get_element_box: bounding_box() failed for %r: %s",
            selector, exc,
        )
        return None
...
def human_scroll_into_view(
    ...
    if not all(k in box for k in ("x", "y", "width", "height")):
        raise RuntimeError("Element not found or has no bounding box")
    ...
    for i in range(total_clicks):
        if i < accel_steps:
            ...
            if cfg.scroll_lognormal_pause:
                median_delta = (cfg.scroll_delta_base[0] + cfg.scroll_delta_base[1]) / 2
                delta = rand_lognormal(median_delta, 0.20)
                delta = max(50, min(200, delta))
                median_pause = (cfg.scroll_pause_fast[0] + cfg.scroll_pause_fast[1]) / 2
                pause = rand_lognormal(median_pause, 0.25)
            else:
        ...
        # Real users pause to read every few scrolls, and occasionally
        # scroll back a bit to re-read something.
        if cfg.read_pause_chance > 0 and i > 2 and i < total_clicks - 2:
            ...
            if rand_unit() < cfg.read_backscroll_chance:
                ...
                _smooth_wheel(raw, -back_px * direction, cfg)
                sleep_ms(rand(200, 500))
                _smooth_wheel(raw, back_px * direction, cfg)
...
def scroll_to_element(
    ...
    """
    # Create the locator once and reuse it for every bounding-box poll inside
    # human_scroll_into_view — avoids re-resolving the selector on each step.
    loc = page.locator(selector).first

    def _get_box() -> Optional[dict]:
        try:
            return loc.bounding_box(timeout=max(1, timeout))
        except _TargetClosedError:
            raise
        except Exception as exc:
            logger.debug(
                "scroll_to_element: bounding_box() failed for %r: %s",
                selector, exc,
            )
            return None

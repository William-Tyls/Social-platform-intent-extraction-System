...
def ensure_actionable(
    ...
    while True:
        ...
        try:
            ...
            # Unexpected error (e.g. transient TargetClosedError, transport
            # hiccup). Back off and retry while the deadline still allows it;
            # otherwise re-raise so the caller sees the real failure.
            logger.debug(
                "ensure_actionable: unexpected error for %r: %s", selector, e
            )
            if time.monotonic() >= deadline:
                raise
            _backoff_sleep(attempt)
...
#
# Both the locator and ElementHandle paths run identical JS (the only difference
# is how the ``expected`` element is supplied — locator.evaluate vs
# handle.evaluate — which is handled by the caller, not the JS body), so a
# single constant covers both.
_POINTER_EVENTS_JS = """(expected, data) => {
...
# Backwards-compatible aliases — the locator and handle bodies used to be two
# identical copies; they now both reference the single shared constant.
_POINTER_EVENTS_LOCATOR_JS = _POINTER_EVENTS_JS
_POINTER_EVENTS_HANDLE_JS = _POINTER_EVENTS_JS
    ...
    while True:
        ...
        if result is None:
            ...
            if consecutive_failures >= 5:
                raise ElementNotReceivingEventsError(
                    selector,
                    f"pointer_events check failed {consecutive_failures} times in a row",
                )
            # Back off and retry — transient evaluate failure.
            if time.monotonic() >= deadline:
                raise ElementNotReceivingEventsError(selector, "unknown")
            _backoff_sleep(attempt)
            attempt += 1
            continue

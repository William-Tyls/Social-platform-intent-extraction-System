...
from .config import rand, rand_range, rand_lognormal, sleep_ms, async_sleep_ms
...
# Sentinel returned by isolated-world evaluate() when evaluation genuinely
# fails (exception / stale context). Distinct from None, which is a legitimate
# return value for many expressions (e.g. `el === document.activeElement` when
# the element is not focused). Callers that need to distinguish "failed to
# evaluate" from "evaluated to a falsy value" check `result is _EVAL_FAILED`.
_EVAL_FAILED = object()
...
class _SyncIsolatedWorld:
    ...
    def evaluate(self, expression: str) -> Any:
        ...
        Returns ``_EVAL_FAILED`` (a module-level sentinel) when evaluation
        could not be completed after a retry, so callers can distinguish a
        genuine failure from a legitimate ``None``/``False`` return value.
        """

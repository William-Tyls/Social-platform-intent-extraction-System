...
from .config import HumanConfig, rand, rand_range, rand_int_range, rand_lognormal, sleep_ms, rand_unit
...
class Point:
    ...
    def __init__(self, x: float, y: float):
        if not (math.isfinite(x) and math.isfinite(y)):
            raise ValueError(f"Point coordinates must be finite (got x={x!r}, y={y!r})")
        self.x = x
        self.y = y
...
def human_move(
    ...
    for i in range(steps + 1):
        ...
        if burst_counter >= burst_size and i < steps:
            if cfg.mouse_lognormal_pause:
                median = (cfg.mouse_burst_pause[0] + cfg.mouse_burst_pause[1]) / 2
                sleep_ms(rand_lognormal(median, 0.25))
            else:
    ...
    if rand_unit() < cfg.mouse_overshoot_chance:
        ...
        sleep_ms(rand(30, 70))
        raw.move(round(end_x + (rand_unit() - 0.5) * 4),

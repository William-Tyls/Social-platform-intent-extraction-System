...
async def _get_element_box_async(
    ...
    try:
        ...
        logger.debug(
            "_get_element_box_async: bounding_box() failed for %r: %s",
            selector, exc,
        )
        return None
...
async def async_human_scroll_into_view(
    ...
    for i in range(total_clicks):
        if i < accel_steps:
            ...
            if cfg.scroll_lognormal_timing:
...
async def async_scroll_to_element(
    ...
    # async_human_scroll_into_view — avoids re-resolving the selector each step.
    loc = page.locator(selector).first

    async def _get() -> Optional[dict]:
        try:
            return await loc.bounding_box(timeout=max(1, timeout))
        except _TargetClosedError:
            raise
        except Exception as exc:
            logger.debug(
                "async_scroll_to_element: bounding_box() failed for %r: %s",
                selector, exc,
            )
            return None

# CloakBrowser Enhanced

[CloakBrowser](https://github.com/CloakHQ/CloakBrowser) 的增强版 fork (v0.3.31, Chrome/145)，在 58 个 C++ 源码级指纹补丁之上，进一步优化了反检测能力和行为拟人化。

在此增强版引擎之上，构建了**多平台社交数据意图提取系统**，支持 **YouTube** / **Twitter/X** / **Reddit** 三大平台的用户内容批量采集和 LLM 智能过滤。
...
| **CreepJS 31%→0%** | API polyfill 修正 | 旧 polyfill 注入目标错误。通过逆向 `creep.js` 源码定位到真实检测点 → 修正为 `window.ContentIndex`、`window.ContactsManager`、`NetworkInformation.prototype.downlinkMax` |
...
**FingerprintJS (L3)** 和 **PixelScan inconsistent** 共享同一根因：CloakBrowser binary 未修补 `chrome.runtime`、`navigator.plugins`(仅5)、`navigator.mimeTypes`(仅2)、`navigator.userAgentData`(macOS 缺失)。C++ 层的 V8 native getter 无法用 JavaScript polyfill 覆盖，需 binary 团队在编译层注入 Compat API。
...
└──────┬───────────────┬───────────────┬───────┘
  ...
  TwitterWorker   RedditWorker   YoutubeWorker
       ...
       │               │               └─ yt-dlp (内容提取)
       │               │
       │               └─ CloakBrowser Enhanced (隐身浏览器)
       └─ CloakBrowser Enhanced (隐身浏览器 + 登录态)

提取层 (纯函数, 可独立使用):
  tools/extractors/
  ├── twitter_extractor.py   — Twitter/X DOM 提取
  ├── reddit_extractor.py    — old.reddit.com HTML 解析
  ├── youtube_api.py         — 官方 API v3 + QuotaTracker (跨进程持久化 + fcntl.flock)
...
> CloakBrowser 引擎 (含本 fork 的优化) 已内置于 `cloakbrowser/` 和 `js/` 目录中，无需额外 `pip install cloakbrowser`。
...
```
├── cloakbrowser/              ← CloakBrowser Python 库 (含本 fork 优化)
├── js/                        ← CloakBrowser JS/TS 库 (含本 fork 优化)
├── BINARY-LICENSE.md          ← CloakBrowser 二进制许可
│
└── tools/                     ← 上层应用
...
`cloakbrowser/` 和 `js/` 遵循上游许可 (wrapper 代码 MIT, 二进制见 [BINARY-LICENSE.md](BINARY-LICENSE.md))。`tools/` 为本项目新增代码，MIT License。
